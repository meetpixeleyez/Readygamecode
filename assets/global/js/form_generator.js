class FormGenerator{
        ...
        if(this.fieldType == 'select') {
            ...
            <div class="form-group mt-2">
                <div class="d-flex gap-2">
                    <input type="checkbox" name="is_multiple" id="is_multiple" class="form-check-input">
                    <label class="form-check-label" for="is_multiple">Enable Multiple Selection</label>
                </div>
            </div>`;
        }
    ...
    formsToJson(form){
        ...
        var formItem = {
            ...
            options:options,
            is_multiple:form.find('[name=is_multiple]').val(),
    ...
    makeFormHtml(formItem,updateId){
        ...
        var hiddenFields = `
            ...
            <input type="hidden" name="form_generator[is_multiple][]" value="${formItem.is_multiple ?? 'false'}">
    ...
    formEdit(element){
        ...
        form.find('[name=is_multiple]').val(formItem.is_multiple).change();
        ...
        if(formItem.type == 'select') {
            ...
            <div class="form-group mt-2">
                <div class="d-flex gap-2">
                    <input type="checkbox" name="is_multiple" id="is_multiple" class="form-check-input" 
                        ${(formItem.is_multiple === true || formItem.is_multiple === "true" || formItem.is_multiple === "1") ? 'checked' : ''}>
                    <label class="form-check-label" for="is_multiple">Enable Multiple Selection</label>
                </div>
            </div>`;
        }

...
export default async function SellerRefundDisputePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="space-y-6 max-w-4xl mx-auto mt-10">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild>
          <Link href="/seller/refunds"><ArrowLeft className="w-5 h-5" /></Link>
        </Button>
        <div>
              ...
              >
                <div className="flex justify-between items-end gap-4 mb-2 opacity-70 text-xs">
                  <span className="font-semibold">
                    {activity.sellerId === session.sub ? "You (Seller)" : "Buyer"}
                  </span>
